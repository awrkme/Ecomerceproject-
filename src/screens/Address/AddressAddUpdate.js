...
import {updateUserAddress} from '../../redux/actions/CartApi';
import {showProgressBar} from '../../redux/actions';

const AddressAddUpdate = ({navigation, route}) => {
  ...
  useEffect(() => {
    ...
    setEmail(item?.email);
    setAddressDetail(item?.address);
    setAddressDefault(item?.defaultAddress == 1);
  }, []);
